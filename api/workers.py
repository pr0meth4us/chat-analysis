    ...
    try:
        ...
        for temp_file_path in temp_file_paths:
            ...
            if is_zipfile(temp_file_path):
                ...
                file_objs.append(f)
        if not file_objs: raise ValueError("No processable files were found in the upload(s).")
        ...
        def file_progress_callback(progress_percent=None, stage=None, message=None, **kwargs):
            if progress_percent is not None:
                ...
                if message: display_stage = f"{display_stage} - {message}"
                update_progress(mapped_progress, display_stage)

        processed_messages = process_uploaded_files(file_objs, progress_callback=file_progress_callback)
